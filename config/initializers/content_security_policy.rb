...
# Application-wide Content Security Policy. script-src is same-origin ONLY (no CDN: nostr-tools and
# @cashu/cashu-ts are vendored self-contained into vendor/javascript, so the key-handling and escrow-
# locking crypto cannot be tampered via a third-party CDN) plus a per-request nonce, with no
# unsafe-inline, so an injected inline script is refused. style-src allows inline because the vendored
# Tailwind Plus Elements components style inline. connect-src is open to https and wss because a Nostr
# client reaches arbitrary user-chosen relays (wss) and NIP-05 domains (https); img-src allows https
# for remote profile pictures.
