...
module Users
	class MetadataFetchTest < ActiveSupport::TestCase
		M = Events::Kinds::METADATA

		def event(pubkey:, created_at:, id: SecureRandom.hex(32), kind: M)
		...
		def pick_from(events, pubkey:)
			...
			Users::MetadataFetch.call(pubkey:)
		ensure
			Relays::FetchEvents.singleton_class.send(:remove_method, :call)
		end

		test "picks the latest of MY kind:0 and filters foreign pubkeys and wrong kinds" do
